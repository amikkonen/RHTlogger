...
def plot(date, T_RTC, T_sht31, T_si7021, RH_sht31, RH_si7021, 
    ...
    if not(T_ref is None):
        ax.plot(date_ref, T_ref, "-", label=ref_name)
...
#    ax.set_ylim(None, 23)
    ...
    if not(RH_ref is None):
        ax.plot(date_ref, RH_ref, "-", label=ref_name)
...
#    ax.plot(date, my_get_ws(T_si7021, RH_si7021), "-", label="my_SI7021")
    ...
    ax.set_title("w")
    ax.set_ylabel('w (g water/m^3)')
    
    
    if not(w_ref is None):
        ax.plot(date_ref, w_ref*1e3, "-", label=ref_name)
...
#        return a*x + b
    ...
    popt, pcov = optimize.curve_fit(fit, sp.arange(len(voltage)), voltage)
#
    ax.plot(date, fit(sp.arange(len(voltage)), *popt), 'g--',
            label="exp fit")
#            label="exp fit" + str(tuple(popt)))
